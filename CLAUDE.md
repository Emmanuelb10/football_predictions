...
- **Scores only from independent result providers**: Ingestion never stores scraped scores — matches inserted as `scheduled` with null scores. `resultSync` compares livescore.com, sofascore.com, ESPN and Flashscore Kenya (with prosoccer/zulubet as limited HTML fallbacks) before writing a final score.
